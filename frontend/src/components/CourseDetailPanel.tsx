...
export default function CourseDetailPanel({ course, lessons, onClose, studentName }: Props) {
  ...
  return (
    <aside className="fixed inset-0 z-20 flex items-start justify-center overflow-y-auto bg-black/30 p-4 sm:p-10">
      <div className="relative w-full max-w-3xl overflow-hidden rounded-3xl border bg-white shadow-2xl">
        <button
          className="absolute right-4 top-4 z-10 rounded-full border border-gray-200 bg-white px-3 py-1 text-xs font-medium text-gray-600 hover:bg-gray-50"
          onClick={onClose}
        >
          Close
        </button>
